...
async def test_add_store_success(authed_client):
    store_name = "test"
    response = await authed_client.post("/api/stores", json={"name": store_name})
...
async def test_add_store_duplicate(authed_client):
    store_name = "test"
    await authed_client.post("/api/stores", json={"name": store_name})
    response = await authed_client.post("/api/stores", json={"name": store_name})
...
async def test_list_stores_success(authed_client, db_session):
    ...
    response = await authed_client.get("/api/stores")
    assert response.status_code == 200
    ...
    response = await authed_client.get("/api/stores", params={"page": 2})
    response_body = response.json()
    assert len(response_body["data"]["stores"]) == 2
    assert response_body["data"]["total"] == 12
...
async def test_update_store_success(authed_client, db_session):
    ...
    response = await authed_client.patch(
        f"/api/stores/{store.id}", json={"name": "test2"}
    )
    assert response.status_code == 200
    response_body = response.json()
